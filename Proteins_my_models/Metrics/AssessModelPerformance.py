...
def batch_metric_sums(targets_np, probs_np, thresh=0.5):
    valid = targets_np.sum(axis=1) > 0
    n = valid.sum()
    if n == 0:
        return None, 0
    t, p = targets_np[valid], probs_np[valid]
    sums = {
        "auprc": metrics.average_precision_score(t, p, average="samples") * n,
        "auroc": metrics.roc_auc_score(t, p, average="samples") * n,
    }
    return sums, n
